...
- Resolved a two-year pattern of near-nightly reporting-database outages that corrupted downstream Snowflake data and forced manual morning re-runs. Diagnosed the bottlenecks with Grafana and MySQL performance counters, then traced root causes across three layers: a non-sargable modulo shard predicate in the Prefect extraction path that multiplied read load ~10x, roughly 15 multi-tenant stored procedures, and InnoDB server configuration. Fixes included window-function rewrites of rolling self-joins, sargable-predicate and join-order corrections, temp-table indexing, and redo-log and flush tuning. The full nightly load now completes with **zero spill to swap** and no hardware upgrade, validated across 60+ tenant schemas with row-count and row-hash parity.
